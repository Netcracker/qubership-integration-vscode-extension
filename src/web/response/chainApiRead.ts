...
export function getMaskedField(chain: any, id: string) {
    ...
    let maskedField = chain.content.maskedFields?.find(
        (mf: any) => mf.id === id,
    );
    if (!maskedField) {
...
export async function getElementsByType(
    ...
    if (elementType === "chain-trigger-2") {
        ...
        for (const chainFile of chainFiles) {
            try {
                const chain = (await fileApi.parseFile(
                    chainFile,
                )) as ChainSchema;
                ...
                console.error(
                    `Failed to parse chain file ${chainFile.path}`,
                    error,
                );
            }
        }
...
async function parseElement(
    ...
    if ((element.children as ElementSchema[])?.length) {
        ...
        for (const child of element.children as ElementSchema[]) {
            children.push(
                await parseElement(fileUri, child, chainId, element.id),
            );
        }
    }
...
async function schemaToChain(
    ...
    return {
        ...
        deployAction: chain.content.deployAction
            ? ChainCommitRequestAction[
                  chain.content
                      .deployAction as keyof typeof ChainCommitRequestAction
              ]
            : undefined,
...
export async function findChainByElementId(
    ...
    const chain = await findChain((chain: ChainSchema) =>
        (chain.content.elements as ElementSchema[]).some((element) => element.id === elementId),
    );
