...
export class FileConversionService {
  ...
  static async fileToSerializedFile(file: File): Promise<SerializedFile> {
    try {
      ...
      throw new Error(
        `Failed to convert File to SerializedFile: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  static serializedFileToFile(serializedFile: SerializedFile): File {
    try {
      ...
      const blob = new Blob([serializedFile.content], {
      ...
      throw new Error(
        `Failed to convert SerializedFile to File: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  static async filesToSerializedFiles(
    files: File[],
    ...
    try {
      ...
      throw new Error(
        `Failed to convert Files to SerializedFiles: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
  ...
  static serializedFilesToFiles(serializedFiles: SerializedFile[]): File[] {
    try {
      return serializedFiles.map((serializedFile) =>
        this.serializedFileToFile(serializedFile),
      );
    } catch (error) {
      throw new Error(
        `Failed to convert SerializedFiles to Files: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
   ...
   */
  static validateSerializedFile(
    serializedFile: any,
  ): serializedFile is SerializedFile {
    return (
      serializedFile &&
