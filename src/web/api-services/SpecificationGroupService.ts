...
export class SpecificationGroupService {
  ...
  async getSpecificationGroupById(
    groupId: string,
    systemId: string,
  ): Promise<SpecificationGroup | null> {
    try {
      ...
      console.error(
        `[SpecificationGroupService] Error getting specification group ${groupId}:`,
        error,
      );
      return null;
    }
   ...
   */
  async saveSpecificationGroupFile(
    systemId: string,
    ...
    try {
      ...
      const config = ProjectConfigService.getConfig();
      const groupFile = Uri.joinPath(
        baseFolder,
        `${specificationGroup.id}${config.extensions.specificationGroup}`,
      );

      console.log(
        `[SpecificationGroupService] Saving specification group file:`,
        {
          systemId,
      ...
      const yamlData = {
        ...
        content: {
          ...
          parentId: systemId,
          labels: specificationGroup.labels
            ? LabelUtils.fromEntityLabels(specificationGroup.labels)
            : [],
        },
      };
      ...
      console.log(
        `[SpecificationGroupService] Saved specification group file: ${groupFile.fsPath}`,
      );
    } catch (error) {
      console.error(
        `[SpecificationGroupService] Error saving specification group file:`,
        {
      ...
      throw new Error(
        `Failed to save specification group file: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }
}
