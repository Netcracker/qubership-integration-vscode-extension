...
export class WsdlParser {
    parse(resources: WsdlResource[], mainUri: string): WsdlParseResult {
        ...
        const targetNamespace = mainRoot.getAttribute("targetNamespace") || undefined;
        ...
        const serviceNames = Array.from(new Set(uniqueEndpoints.map((endpoint) => endpoint.serviceName).filter(Boolean))) as string[];
        ...
        operationArray.forEach((operationName) => {
            const details = operationDetails.get(operationName) ?? { name: operationName };
    ...
    private collectOperationsV11(root: Element, operations: Set<string>): void {
        this.traverseElements(root, (element) => {
            if (element.namespaceURI === WSDL_11_NS && element.localName === "operation") {
                const parent = element.parentNode;
                if (parent && (parent as Element).namespaceURI === WSDL_11_NS && (parent as Element).localName === "binding") {
    ...
    private collectOperationsV20(root: Element, operations: Set<string>): void {
        this.traverseElements(root, (element) => {
            if (element.namespaceURI === WSDL_20_NS && element.localName === "binding") {
                this.forEachChildElement(element, (child) => {
                    if (child.namespaceURI === WSDL_20_NS && child.localName === "operation") {
    ...
    private collectEndpointsV20(root: Element, endpoints: WsdlEndpoint[]): void {
        this.traverseElements(root, (element) => {
            if (element.namespaceURI === WSDL_20_NS && element.localName === "service") {
                const serviceName = element.getAttribute("name") || undefined;

                this.forEachChildElement(element, (endpointElement) => {
                    if (endpointElement.namespaceURI === WSDL_20_NS && endpointElement.localName === "endpoint") {
                        const endpointName = endpointElement.getAttribute("name") || undefined;
                        const address = endpointElement.getAttribute("address") || undefined;
    ...
    private collectMessagesV11(root: Element, messages: Map<string, WsdlMessage>): void {
        if (root.namespaceURI !== WSDL_11_NS) {
            return;
        }
        const definitionsTargetNamespace = root.getAttribute("targetNamespace") || undefined;

        this.forEachChildElement(root, (element) => {
            if (element.namespaceURI === WSDL_11_NS && element.localName === "message") {
                const messageName = element.getAttribute("name");
                if (!messageName) {
                    return;
                }

                const key = this.buildQualifiedNameKey({ namespace: definitionsTargetNamespace, name: messageName });
                if (messages.has(key)) {
                    return;
                }
                ...
                this.forEachChildElement(element, (partElement) => {
                    if (partElement.namespaceURI === WSDL_11_NS && partElement.localName === "part") {
                        ...
                        if (elementAttr) {
                            part.element = this.resolveQualifiedName(partElement, elementAttr);
                        }
                        if (typeAttr) {
    ...
    private collectOperationMessagesV11(root: Element, operationDetails: Map<string, WsdlOperationDetails>): void {
        this.forEachChildElement(root, (element) => {
            if (element.namespaceURI === WSDL_11_NS && element.localName === "portType") {
                this.forEachChildElement(element, (operationElement) => {
                    if (operationElement.namespaceURI === WSDL_11_NS && operationElement.localName === "operation") {
                        const operationName = operationElement.getAttribute("name");
                        if (!operationName) {
                            return;
                        }
                        const existing = operationDetails.get(operationName) ?? { name: operationName };
                        const inputElement = this.findFirstChildElement(operationElement, WSDL_11_NS, "input");
                        const outputElement = this.findFirstChildElement(operationElement, WSDL_11_NS, "output");

                        if (inputElement) {
                            existing.input = this.resolveQualifiedName(inputElement, inputElement.getAttribute("message"));
                        }
                        if (outputElement) {
                            existing.output = this.resolveQualifiedName(outputElement, outputElement.getAttribute("message"));
                        }
    ...
    private collectOperationMessagesV20(root: Element, operationDetails: Map<string, WsdlOperationDetails>): void {
        this.traverseElements(root, (element) => {
            if (element.namespaceURI === WSDL_20_NS && element.localName === "interface") {
                this.forEachChildElement(element, (operationElement) => {
                    if (operationElement.namespaceURI === WSDL_20_NS && operationElement.localName === "operation") {
                        const operationName = operationElement.getAttribute("name");
                        if (!operationName) {
                            return;
                        }
                        ...
                        this.forEachChildElement(operationElement, (child) => {
                            ...
                            if (child.localName === "input") {
                                existing.input = this.resolveQualifiedName(child, child.getAttribute("element"));
                            } else if (child.localName === "output") {
                                existing.output = this.resolveQualifiedName(child, child.getAttribute("element"));
                            }
                        });
                        operationDetails.set(operationName, existing);
                    }
    ...
    private collectSchemas(root: Element, uri: string | undefined, schemas: WsdlSchemaEntry[]): void {
        this.forEachChildElement(root, (element) => {
            if ((element.namespaceURI === WSDL_11_NS || element.namespaceURI === WSDL_20_NS) && element.localName === "types") {
                this.forEachChildElement(element, (schemaElement) => {
                    if (schemaElement.namespaceURI === XML_SCHEMA_NS && schemaElement.localName === "schema") {
                        schemas.push({
                            uri,
    ...
    private resolveQualifiedName(node: Element, value: string | null): WsdlQualifiedName | undefined {
        if (!value) {
            return undefined;
        }
    ...
    private findChildAddress(portElement: Element): Element | undefined {
        ...
        while (current) {
            if (current.nodeType === 1) {
                ...
                if (childElement.localName === "address" && (childElement.namespaceURI === SOAP_11_NS || childElement.namespaceURI === SOAP_12_NS)) {
                    return childElement;
                }
            }
    ...
    private findFirstChildElement(element: Element, namespace: string, localName: string): Element | undefined {
    ...
    private traverseElements(element: Element, visitor: (element: Element) => void) {
        visitor(element);
        this.forEachChildElement(element, (child) => this.traverseElements(child, visitor));
    }
