...
export type WsdlResolver = (
  uri: string,
  baseUri: string,
) => Promise<WsdlResource | null>;
...
export class WsdlLoader {
  ...
  private async collectRecursive(
    uri: string,
    content: string,
    visited: Map<string, WsdlResource>,
  ): Promise<void> {
    if (visited.has(uri)) {
      return;
    }
    ...
    for (const importUri of imports) {
      ...
      if (!resolved) {
        console.warn(
          "[WsdlLoader] Unable to resolve import",
          importUri,
          "from",
          uri,
        );
  ...
  private extractImports(content: string): string[] {
    const regex =
      /<(?:[a-zA-Z0-9_]+:)?(?:import|include)[^>]*(?:schemaLocation|location)\s*=\s*(?:"([^"]+)"|'([^']+)')[^>]*>/gi;
