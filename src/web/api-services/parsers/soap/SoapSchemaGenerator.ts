...
export class SoapSchemaGenerator {
  ...
  buildOperationSchemas(): Map<string, SoapOperationSchemas> {
    ...
    Object.entries(this.wsdlData.operationDetails).forEach(
  ...
  private resolveMessage(
    ...
    const nameOnlyKey = SoapSchemaGenerator.buildQualifiedNameKey({
      namespace: undefined,
    ...
    return Object.values(this.wsdlData.messages).find(
      (message) => message.name === messageRef.name,
    );
  }

  private buildSchemaForMessagePart(
    part: WsdlMessagePart,
    context: SchemaBuildContext,
  ): SequenceElementSchema | undefined {
    if (part.element) {
  ...
  private buildSchemaFromElement(
    qName: WsdlQualifiedName,
    context: SchemaBuildContext,
  ): any {
    ...
    const definitionName = this.generateDefinitionName(
      "Element",
      qName,
      context,
    );
  ...
  private buildElementSchemaBody(
    element: Element,
    context: SchemaBuildContext,
  ): any {
  ...
  private buildSchemaForTypeReference(
    qName: WsdlQualifiedName,
    context: SchemaBuildContext,
  ): any {
  ...
  private buildComplexTypeSchema(
    complexType: Element,
    context: SchemaBuildContext,
  ): any {
    ...
    if (choice) {
      ...
      this.forEachChildElement(choice, (child) => {
        if (
          child.namespaceURI === XML_SCHEMA_NS &&
    ...
    const simpleContent = this.findFirstChildElement(
      complexType,
      "simpleContent",
    );
    if (simpleContent) {
      ...
      if (extension) {
        ...
        const baseType = baseAttr
          ? this.resolveQualifiedName(extension, baseAttr)
          : undefined;
        if (baseType) {
    ...
    const complexContent = this.findFirstChildElement(
      complexType,
      "complexContent",
    );
    if (complexContent) {
      ...
      if (extension) {
        ...
        const baseType = baseAttr
          ? this.resolveQualifiedName(extension, baseAttr)
          : undefined;
        let baseSchema: any = baseType
          ? this.buildSchemaForTypeReference(baseType, context)
          : { type: "object" };
        const sequenceChild = this.findFirstChildElement(extension, "sequence");
        if (sequenceChild) {
          const sequenceSchema = this.buildSequenceSchema(
            sequenceChild,
            context,
          );
  ...
  private buildSequenceSchema(
    ...
    this.forEachChildElement(sequence, (child) => {
      ...
      index += 1;
      const elementSchema = this.buildSequenceElementSchema(
        child,
        context,
        index,
      );
  ...
  private buildSequenceElementSchema(
    element: Element,
    context: SchemaBuildContext,
    index: number,
  ): SequenceElementSchema | undefined {
    ...
    const isArray =
      ...
      (maxOccursAttr ? parseInt(maxOccursAttr, 10) > 1 : false);
  ...
  private buildAnonymousElementSchema(
    element: Element,
    context: SchemaBuildContext,
  ): any {
    const typeAttr = element.getAttribute("type");
  ...
  private buildSimpleTypeSchema(
    ...
    if (restriction) {
      ...
      const baseType = baseAttr
        ? this.resolveQualifiedName(restriction, baseAttr)
        : undefined;
      const schema = baseType
        ? this.mapPrimitiveType(baseType)
        : { type: "string" };
    ...
    if (listElement) {
      ...
      const itemType = itemTypeAttr
        ? this.resolveQualifiedName(listElement, itemTypeAttr)
        : undefined;
      const itemSchema = itemType
        ? this.buildSchemaForTypeReference(itemType, context)
        : { type: "string" };
      return {
        type: "array",
        items: itemSchema,
  ...
  private mergeObjectSchemas(baseSchema: any, extensionSchema: any): any {
    ...
    (extensionSchema.required ?? []).forEach((item: string) =>
      required.add(item),
    );
    if (required.size > 0) {
  ...
  private resolveElementQualifiedName(
    element: Element,
  ...
  private getSchemaTargetNamespace(node: Element): string | undefined {
    ...
    while (current) {
      if (
        current.namespaceURI === XML_SCHEMA_NS &&
        current.localName === "schema"
      ) {
        const ns = current.getAttribute("targetNamespace");
        return ns || undefined;
      }
      current =
        current.parentNode instanceof Element
  ...
  private generateDefinitionName(
    prefix: string,
    qName: WsdlQualifiedName,
    context: SchemaBuildContext,
  ): string {
    const ns = qName.namespace
      ? qName.namespace.replace(/[^A-Za-z0-9]+/g, "_")
      : "default";
    let baseName = `${prefix}_${ns}_${qName.name}`.replace(
      /[^A-Za-z0-9_]+/g,
      "_",
    );
    if (!baseName) {
...
class XmlSchemaStore {
  ...
  getType(
    qName: WsdlQualifiedName,
  ): { kind: "complex" | "simple"; element: Element } | undefined {
