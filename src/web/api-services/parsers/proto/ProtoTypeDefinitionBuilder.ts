...
export class ProtoTypeDefinitionBuilder {
  ...
  private static buildFieldSchema(field: protobuf.Field): JsonSchema {
    if (field.map) {
      return {
        type: "object",
        additionalProperties: this.buildTypeNode(
          field.type,
          field.resolvedType,
        ),
      };
  ...
  private static buildTypeNode(
    typeName: string,
    resolvedType: protobuf.ReflectionObject | null | undefined,
  ): JsonSchema {
    switch (typeName) {
      case "double":
      case "float":
      ...
      default: {
        if (resolvedType && "fullName" in resolvedType) {
          const fullName = this.normalizeFullName(
            (resolvedType as protobuf.ReflectionObject).fullName ?? typeName,
          );
  ...
  private static getJsonName(field: protobuf.Field): string {
    ...
    const optionName =
      options && typeof options["json_name"] === "string"
        ? (options["json_name"] as string)
        : undefined;
