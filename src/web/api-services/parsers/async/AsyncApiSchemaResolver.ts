...
export class AsyncApiSchemaResolver {
    resolveRef(schemaRef: string, components: JsonValue): ResolvedSchema | null {
        ...
        const schemaRefs = this.getNestedRefs(schemaNode, clonedComponents, new Set());
        const resolvedSchema = this.getResolvedSchema(schemaRef, schemaNode, schemaRefs);
    ...
    private getSchemaNode(schemaRef: string, componentsNode: JsonValue): JsonValue {
    ...
    private getResolvedSchema(schemaRef: string, schemaNode: JsonValue, schemaRefs: Map<string, JsonValue>): JsonValue {
    ...
    private getNestedRefs(schemaNode: JsonValue, componentsNode: JsonValue, visited: Set<string>): Map<string, JsonValue> {
        ...
        if (schemaType === OBJECT_FIELD_TYPE) {
            ...
            this.collectSchemaNodeProperties(result, propertiesNode, componentsNode, visited);

            const additionalNode = schemaNode[ADDITIONAL_PROPERTIES_FIELD_NAME];
            this.collectSchemaNodeProperties(result, additionalNode, componentsNode, visited);
        } else if (schemaType === ARRAY_FIELD_TYPE && schemaNode[ITEMS_FIELD_NAME]) {
            const itemsNode = schemaNode[ITEMS_FIELD_NAME];
            if (isObject(itemsNode) && typeof itemsNode[REF_FIELD_NAME] === "string") {
                ...
                if (!visited.has(definitionKey)) {
                    ...
                    const refNode = this.getSchemaNodeFromComponents(itemsNode[REF_FIELD_NAME] as string, componentsNode);
            ...
            if (!visited.has(definitionKey)) {
                visited.add(definitionKey);
                const refNode = this.getSchemaNodeFromComponents(refKey, componentsNode);
                this.convertPayloadToSchemaNode(refNode);
                result.set(definitionKey, refNode);
                const nestedRefs = this.getNestedRefs(refNode, componentsNode, visited);
                nestedRefs.forEach((value, key) => result.set(key, value));
    ...
    private collectSchemaNodeProperties(
        ...
        if (Array.isArray(node)) {
            node.forEach((item) => {
                if (isObject(item)) {
                    this.collectSchemaNodeProperties(target, item, componentsNode, visited);
                }
            });
            return;
        }
        ...
        if (isObject(node)) {
            const iterableRefs = this.getIterablePropertyRefs(node, componentsNode, visited);
            ...
            Object.values(node).forEach((value) => {
                if (isObject(value)) {
                    this.collectSchemaNodeProperties(target, value, componentsNode, visited);
    ...
    private getIterablePropertyRefs(
        ...
        const result = new Map<string, JsonValue>();
        const iterableField = [PROPERTIES_FIELD_NAME, ALL_OF_FIELD_NAME, ANY_OF_FIELD_NAME].find(
            (field) => field in property
        );
    ...
    private getRefs(
        ...
        if (typeof property[REF_FIELD_NAME] === "string") {
            ...
            if (!visited.has(definitionKey)) {
                visited.add(definitionKey);
                const componentNode = this.getSchemaNodeFromComponents(rewritten, componentsNode);
                this.convertPayloadToSchemaNode(componentNode);
                result.set(definitionKey, componentNode);
                const nestedRefs = this.getNestedRefs(componentNode, componentsNode, visited);
                nestedRefs.forEach((value, key) => result.set(key, value));
            }
        }
    ...
    private getSchemaNodeFromComponents(ref: string, componentsNode: JsonValue): JsonValue {
