      ...
      try {
        if (!FileParserService.hasTextMethod(file)) {
          resolve({
            valid: false,
            errors: [
              { path: "file", message: "File.text() method not available" },
            ],
          });
          return;
        }
        ...
        if (!schemaType) {
          resolve({
            ...
            errors: [
              {
                path: "$schema",
                message: `Unknown QIP schema: ${content.$schema}`,
              },
            ],
          });
          return;
        ...
        if (!this.ajv) {
          ...
          this.ajv.addSchema(QIP_SCHEMAS.SPECIFICATION, "specification");
          this.ajv.addSchema(
            QIP_SCHEMAS.SPECIFICATION_GROUP,
            "specification-group",
          );
          this.ajv.addSchema(QIP_SCHEMAS.SERVICE, "service");
          this.ajv.addSchema(QIP_SCHEMAS.CHAIN, "chain");
        ...
        if (!validate) {
          resolve({
            ...
            errors: [
              {
                path: "schema",
                message: `Schema validator not found for type: ${schemaType}`,
              },
            ],
          });
          return;
  ...
  static getValidationStats(results: Map<string, ValidationResult>): {
    ...
    for (const result of results.values()) {
      ...
      if (result.schemaType) {
        stats.byType[result.schemaType] =
          (stats.byType[result.schemaType] || 0) + 1;
      }
    }
